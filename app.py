...
def create_monthly_metrics_plot(top_n=5):
    ...
    if top_n is not None and top_n > 0 and metrics.get('assistants'):
        # Calculate total membership events for each assistant
        agent_totals = []
    ...
    for idx, agent_name in enumerate(assistants):
        ...
        # Add bar trace for total members
        # Only show bars for months where assistant has members
        x_bars = []
        y_bars = []
        for month, count in zip(months, agent_data['total_members']):
            if count > 0:  # Only include months with members
                x_bars.append(month)
                y_bars.append(count)
...
def get_leaderboard_dataframe():
    ...
    print(f"Filtered out {filtered_count} assistants with 0 total membership events")
...
print(f"\nStarting SWE Assistant Member Leaderboard")
...
with gr.Blocks(title="SWE Assistant Member Leaderboard", theme=gr.themes.Soft()) as app:
    gr.Markdown("# SWE Assistant Member Leaderboard")
    gr.Markdown(f"Track and compare total membership events by SWE assistants")

    with gr.Tabs():

        # Leaderboard Tab
        with gr.Tab("Leaderboard"):
            gr.Markdown("*Statistics are based on total membership events by assistants*")
            ...
            with gr.Group():
                ...
                gr.Markdown("*Shows total membership events for the most active assistants*")
